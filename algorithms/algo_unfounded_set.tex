\begin{algorithm}[t]\caption{\textsc{UnfoundedSet}\label{algo:unfounded:set}}
\BlankLine
...
\BlankLine
$S:=\{a\in\atom{P}\setminus\flits{\ass} \mid \source{a} \in \flits{\ass}\cup\{\undef\}\}$\AlgoComm*{initialize scope $S$}
\Repeat{$T=\emptyset$}{%
...
\BlankLine
\lWhile{$S\neq\emptyset$}{%
  \Let(\AlgoComm*[f]{select starting point $a$}){$a\in S$}{%
    $U:=\{a\}$\;
    \Repeat{$U=\emptyset$}{%
      \lIf{$\EB{U}{P}\subseteq\flits{\ass}$}{%
        \Return $U$\AlgoComm*{(non-empty) unfounded set}
      }
      \Let{$B\in \EB{U}{P}\setminus\flits{\ass}$}{%
